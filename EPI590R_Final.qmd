...
# The above is a cross-reference and a table showing the descriptive statistics for 'Case Count', 'Number Screened', and 'Percent of All Disorders in Region' including an overall column #
...
2.  Fit a regression and present well-formatted results from the regression (1 pt).
    ...
    # The above is a cross-reference and a linear regression results table #
...
3.  Create a figure (1 pt).
    ...
    # The above is a cross-reference and a histogram of the case distribution #
    ```

    The distribution of `Case Count` is right-skewed, with most regions having a relatively low number of cases and a few having a high number of cases, as shown in @fig-hists-1.
    ...
    # The above is a cross-reference and a scatter plot between Percent of All Disorders in Region and Case Count #
...
4.  Write *and* use a function that does something with the data (1 pt)
    ...
    The mean number of cases is `r case_stats$mean` while the standard deviation of cases is `r case_stats$sd`.
