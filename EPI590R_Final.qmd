...
# Convert percent_region column to numeric & create factor for categorical variable: region #
  mutate(
...
# The above is a cross-reference for a table displaying the descriptive statistics for 'Case Count', 'Number Screened', and 'Percent of All Disorders in Region' including an overall column #

# Inline descriptive statistics #
table1 <- tbl_summary(
  nsdca_cleaned,
  by = region,
   include = c(region, cases, screened, percent_region)) |> 
...
Descriptive statistics for the `Case Count`, `Number Screened`, and `California Region` variables were presented in @tbl-one.

There were `r total_sample` California regions observed.
...
2.  Fit a regression and present well-formatted results from the regression (1 pt).
    ...
    # The above is a cross-reference for a linear regression results table #

    # Regression results using {gtsummary} #
    tbl_regression(linear_model, 
                   intercept = TRUE, 
                   label = list(screened ~ "Number Screened"))
    ```

    The results shown in @tbl-two indicated the number of screened newborns was a statistically significant predictor of the number of cases.
...
3.  Create a figure (1 pt).
    ...
    # The above is a cross-reference for a histogram of the case distribution #
    ```

    The distribution of `Case Count` was right-skewed, with most regions having a relatively low number of cases and a few having a high number of cases, as shown in @fig-hists-1.
    ...
    color = "Region")
    # The above is a cross-reference for a scatter plot between 'Percent of All Disorders' in 'Region and Case Count' #

    # Cross-reference #
    total_regions <- n_distinct(nsdca_cleaned$region)
    print(total_regions)
    ```

    @fig-hists-2 is a scatter plot of the `Percent of All Disorders in Region` and `Case Count` variables across `r total_regions` regions.
...
4.  Write *and* use a function that does something with the data (1 pt).
    ...
    case_stats <- calculate_stats(nsdca_cleaned, cases)
    ...
    The mean number of cases was `r case_stats$mean` while the standard deviation of cases was `r case_stats$sd`.
...
5.  The table, regression results, and figure, with appropriate captions (1 pt).
    ...
    The regression analysis revealed that the `Case Count` variable was a significant predictor of the outcome, with a p-value of `r gtsummary::inline_text(screened_table, variable = "cases", pattern = "{p.value}")`.

6.  Read in a data set and save a file (can be data, table, figure, etc.) Use the `{here}` package every time you refer to file paths (at least twice) (1 pt).
